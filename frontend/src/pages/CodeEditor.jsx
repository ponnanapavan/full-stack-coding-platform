...
import problemStarterCode from '@/utils/problemstratedCode';
import { LoaderCircle } from 'lucide-react';
import { toast } from 'sonner';
import { useParams } from 'react-router-dom';

const CodeEditor = ({ questionName }) => {
  const [code, setCode] = useState(problemStarterCode[questionName] || '');
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:4000/api/submitCode/${id}`, {
        method: 'POST',
        headers: {
